...
<a href="{{ route('teachers.students.create', ['teacher' => $teacher->id, 'grade' => $grade->id]) }}" class="btn btn-dark">
        ...
        <tr>
            ...
            <td>
                @foreach($student->studentTeacher as $st)
                    {{ $st->teacher->name ?? '-' }} — {{ $st->subject->name ?? '-' }}<br>
                @endforeach
            </td>
            <td>
            ...
            <td>
                ...
                <a href="{{ route('teachers.payments.index', ['teacher' => $teacher->id, 'student' => $student->id]) }}" 
                   class="btn btn-warning btn-sm">إدارة الدفع</a>
